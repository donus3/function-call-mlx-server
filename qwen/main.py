...
def convert_chat(messages: list[dict], role_mapping: dict | None = None):
...
def transform_to_json_if_xml_like(input_string):
    """
    Transforms a string to JSON if it contains <tag>value</tag> pairs.
    Handles multi-line content between tags.
    If no such pairs are found, it returns the original string.
    """
    if not isinstance(input_string, str):
        return input_string

    # Use re.DOTALL flag to make '.' match newline characters
    matches = re.findall(r"<(\w+)>(.*?)</\1>", input_string, flags=re.DOTALL)
    ...
    data_dict = {key: value.strip() for key, value in data_dict.items()}
...
def preprocess_message(message, functions):
    ...
    it attempts to parse that string into a dictionary.
    ...
    """
    # Safely get the list of tool_calls, default to None if not found
    tool_calls = message.get("tool_calls")

    # Check if tool_calls exists and is a non-empty list
    if tool_calls and isinstance(tool_calls, list):
        # Iterate through each tool_call in the list
        for tool_call in tool_calls:
            # Safely get the function dictionary
...
class PromptCache:
    ...
    model_key: tuple[str, str|None, str|None] | None = ("", None, None)
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def convert_custom_format_to_json(self, input_string: str) -> str:
        ...
        Args:
            input_string: A string containing the custom <function> and <parameter> tags.

        Returns:
            A JSON formatted string, or an empty string if parsing fails.
        """

        def fix_typescript_json_to_python_dict(ts_json_str: str):
            """
            ...
            # Ensure true false
            result = re.sub(r"(:\s*['\"])?\b(True|False)\b(['\"])?", lambda m: m.group(2).lower(),result)
        ...
        try:
            ...
            function_match = re.search(r"<function=([^>]+)>", input_string)
            if not function_match:
                print("Error: Could not find <function=...> tag.")
                return ""
            function_name = function_match.group(1).strip()

            # --- 2. Extract all parameters ---
            # Find all occurrences of the pattern <parameter=key>value</parameter>
            # re.DOTALL makes '.' match newlines, which is crucial for multi-line parameter values.
            ...
            # Use secrets for cryptographically strong random strings.
            # We generate 6 bytes, which results in a 12-character hex string.
    ...
    def generate_response(
        ...
        Args:
            ...
            finish_reason (Literal["length", "stop"] | None): The reason the
        ...
        def parse_function(tool_text):
            tool_calls = self.convert_custom_format_to_json(tool_text.strip())
            return tool_calls
